...
public class VideoEncoderThread extends Thread {
    ...
    public static int IMAGE_HEIGHT = PrefSingleton.getInstance().getInt("height");
    public static int IMAGE_WIDTH = PrefSingleton.getInstance().getInt("width");
    ...
    private static byte[] mFrameData;
    ...
    private void encodeFrame(byte[] input) {
        ...
        mFrameData = NV21toI420SemiPlanar(input,  this.mWidth, this.mHeight);
    ...
    private static byte[] FrameData = new byte[PrefSingleton.getInstance().getInt("width") * PrefSingleton.getInstance().getInt("height") * 3 / 2];
    private static byte[] NV21toI420SemiPlanar(byte[] nv21bytes, int width, int height) {
        byte[] i420bytes = new byte[width * height * 3 / 2];
        try {
            System.arraycopy(nv21bytes, 0, i420bytes, 0, width * height);
        } catch (ArrayIndexOutOfBoundsException e){}
        for (int i = 0; i < nv21bytes.length; i += 2) {
            i420bytes[i] = nv21bytes[i + 1];
            i420bytes[i + 1] = nv21bytes[i];
        }
        return i420bytes;
