...
public class Ch34ReadThread extends Thread {
    ...
    public void run() {
        ...
        while (true) {
            do {
                if (!this.ch34xUARTDriver.isNullUsb()) {
                    return;
                }
            ...
            for (i = 0; i < CH34xUARTDriver.REQUEST_COUNT; ++i) {
                ...
                if (usbRequest == usbRequestI) {
                    byte[] buff = this.ch34xUARTDriver.getByteBuffers()[i].array();
                    int buff_len = this.ch34xUARTDriver.getByteBuffers()[i].position();
                    if (buff_len > 0) {
                        try {
                        ...
                        for (int j = 0; j < buff_len; j++) {
                            this.ch34xUARTDriver.getReadBuffer()[this.ch34xUARTDriver.getNewSpliceIndex()] = buff[j];
                        ...
                        Log.e(CH34xUARTDriver.TAG, "读取错误，数据长度=" + buff_len);
                    }
                    this.ch34xUARTDriver.getUsbRequests()[i].queue(this.ch34xUARTDriver.getByteBuffers()[i], this.ch34xUARTDriver._32);
                } else {
                    if (false) {
