...
public class CH34xUARTDriver {
    ...
    /** USB发送与接收控制器,此类用于向USB设备发送和接收数据以及控制消息。 此类的实例由{@link UsbManager#openDevice}创建 */
    ...
    /** 芯片支持的USB转串口线集合 供应商ID：设备的产品ID，1a86:7523/1a86:5523/1a86:5512 */
    private ArrayList supportVendorProduct = new ArrayList();
    /** 芯片支持的USB转串口种类个数 供应商ID：设备的产品ID，1a86:7523/1a86:5523/1a86:5512 */
    ...
    /** USB请求数据包的类。这可用于在{@link UsbDeviceConnection}中读写数据。UsbRequests可用于在批量和中断端点上传输数据。批量端点上的请求可以通过{@link UsbDeviceConnection#bulkTransfer}同步发送，也可以通过{@link UsbRequest#queue}和{@link UsbDeviceConnection#requestWait}异步发送。中断端点上的请求仅异步发送和接收。<p>此类不支持端点0上的请求； 请将{@link UsbDeviceConnection#controlTransfer}用于终结点零请求。 */
    ...
    public UsbDevice enumerateDevice() {
        ...
        if ((var1 = this.usbManager.getDeviceList()).isEmpty()) {
            if (showToast) {
                Toast.makeText(this.context, "没有设备或设备不匹配!", Toast.LENGTH_LONG).show();
            } else {
                Log.d(TAG, "没有设备或设备不匹配!");
            }
            return null;
        } else {
    ...
    /**
     * 枚举并打开 CH34x 设备（操作流程为：resumeUsbList（或者 EnumerateDevice 后 OpenDevice）， UartInit， SetConfig，
     * 以上流程执行完后即可进行串口收发）
     * <p>这个函数包含了
     *
